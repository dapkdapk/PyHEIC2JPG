...
def convert_single_file(
    ...
    try:
        with Image.open(heic_path) as image:
            ...
            image.save(
                jpg_path, "JPEG", quality=output_quality, exif=exif_data, optimize=True
            )
...
def convert_heic_to_jpg(
    ...
    if os.path.exists(jpg_dir):
        user_input = input(
            f"Existing output folder '{jpg_dir}' detected. Delete and proceed? (y/n): "
        )
    ...
    for heic_path in heic_files:
        ...
        if os.path.exists(jpg_path):
            logger.info(
                "Skipping '%s' as the JPG already exists.", os.path.basename(heic_path)
            )
            continue
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {
            executor.submit(
                convert_single_file, heic_path, jpg_path, output_quality, resize
            ): heic_path
        ...
        for future in as_completed(future_to_file):
            ...
            try:
                ...
                print(
                    f"Conversion progress: {progress}% ({num_converted}/{len(tasks)})",
                    end="\r",
                    flush=True,
                )
            except Exception as e:
                logger.error(
                    "Error occurred during conversion of '%s': %s", heic_file, e
                )
...
@click.command()
@click.argument("heic_dir", nargs=1, type=str, required=True)
@click.option(
    "-q",
    "--quality",
    type=int,
...
@click.option(
    "-r", "--recursive", is_flag=True, help="Search for HEIC files in subdirectories."
)
@click.option(
    "--resize",
    type=str,
...
def main(
    heic_dir, quality, workers, output, recursive, resize, delete_originals, verbose
):
    """
    ...
    Example usage:\n
    heic2jpg /path/to/your/heic/images -q 90 -w 8\n
    heic2jpg /path/to/your/heic/images --resize 1920x1080 -o /custom/output/path
    """
    ...
    result = convert_heic_to_jpg(
        heic_dir, quality, workers, output, recursive, resize, delete_originals
    )
